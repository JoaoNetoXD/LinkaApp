...
test('payment references reject PostgREST filter syntax', () => {
  assert.equal(isValidPaymentReference('empreende_abc-123'), true);
...
test('provider payment must match order reference, currency, and amount', () => {
  const intent = { external_reference: 'empreende_order_1', amount: '10.20' };
  const payment = { external_reference: 'empreende_order_1', transaction_amount: 10.2, currency_id: 'BRL' };
